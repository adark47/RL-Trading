...
def evaluate_agent(
    ...
    for ep in tqdm(range(num_episodes), total=num_episodes, desc=f"{split_name} in episodes", leave=False):  # ✅ Исправлено
    ...
    metrics = {
        ...
        f"{split_name}_all_reward": rewards,  # ✅ Добавлено
        f"{split_name}_all_win_rate": win_rates,  # ✅ Добавлено
    }
...
def main(cfg: MasterConfig = None):
    ...
    with mlflow.start_run(run_name=session_name) as run:
        ...
        mlflow.log_params({
            ...
            "episodes": cfg.trainlog.episodes,
            ...
            "val_freq": cfg.trainlog.val_freq,
            "val_selection_metrics": cfg.trainlog.val_selection_metrics,
            "num_val_ep": cfg.trainlog.num_val_ep,
            "use_final_model": cfg.debug.use_final_model,
        ...
        for ep in counter:
            ...
            history["epsilons"].append(agent.eps_end + (agent.eps_start - agent.eps_end) * np.exp(-agent.total_steps / agent.eps_frames))
            win_rate = info.get("episode_win_rate", 0.0)
            history["win_rates"].append(win_rate)
            # Обновление очередей
            episode_rewards_deque.append(ep_reward)
            history["mean_rewards_N"].append(float(np.mean(episode_rewards_deque)))
            episode_losses_deque.append(avg_loss)
            history["mean_losses_N"].append(float(np.mean(episode_losses_deque)) if episode_losses_deque else 0.0)
            episode_win_rate_deque.append(win_rate)
            history["mean_win_rates_N"].append(float(np.mean(episode_win_rate_deque)) if episode_win_rate_deque else 0.0)
            counter.desc = f"Training loss={avg_loss:.7f}, reward={ep_reward:.5f}"
            # Логируем метрики в MLflow
            mlflow.log_metrics({
                "train_reward": ep_reward,
                "train_loss": avg_loss,
                "epsilon": history["epsilons"][-1],
                "train_win_rate": win_rate,
                "train_mean_reward_N": history["mean_rewards_N"][-1],
                "train_mean_loss_N": history["mean_losses_N"][-1],
                "train_mean_win_rate_N": history["mean_win_rates_N"][-1],
            }, step=ep)
            # Валидация
            if val_env and ep % cfg.trainlog.val_freq == 0:
                ...
                mlflow.log_metrics({
                    "val_mean_reward": metrics["Validation_mean_reward"],
                    "val_mean_pnl": metrics["Validation_mean_pnl"],
                    "val_win_rate": metrics["Validation_win_rate"],
                }, step=ep)
                if val_metric > best_val_metric:
                    best_val_metric = val_metric
                    best_path = os.path.join(models_dir, "best.pth")
                    agent.save_model(best_path)
                    mlflow.log_metric("best_val_metric", val_metric, step=ep)
                    logging.info(
        ...
        if test_seqs:
            ...
            mlflow.log_metrics({
                "test_mean_reward": test_metrics["Test_mean_reward"],
                "test_mean_pnl": test_metrics["Test_mean_pnl"],
                "test_win_rate": test_metrics["Test_win_rate"],
            })
            plot_test_distributions(test_metrics, plots_dir)
            mlflow.log_artifacts(plots_dir)
            # Логируем модель - ИСПРАВЛЕНО: q_network заменен на online_net
            if hasattr(agent, 'online_net'):
                mlflow.pytorch.log_model(agent.online_net, "model")
            elif hasattr(agent, 'policy_net'):
                mlflow.pytorch.log_model(agent.policy_net, "model")
            else:
                logging.warning("Could not find neural network attribute in agent. Using artifact logging instead.")
                mlflow.log_artifact(model_path, "best_or_final_model")
            mlflow.log_artifact(model_path, "best_or_final_model")
