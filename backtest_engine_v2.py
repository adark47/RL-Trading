...
def flatten_config(config, parent_key='', sep='.'):
    ...
    items = []
    for k, v in vars(config).items() if not isinstance(config, dict) else config.items():
    ...
    with mlflow.start_run() as run:
        try:
            ...
            for key, value in flat_config.items():
                # MLflow требует, чтобы параметры были строками
                mlflow.log_param(key, str(value))

            # Логирование информации о запуске
            logging.info(f"[MLflow] Started run with ID: {run.info.run_id} at {mlflow.get_tracking_uri()}")

        except Exception as e:
            logging.warning(f"[MLflow] Failed to initialize: {str(e)}. Continuing without MLflow tracking.")

        # Основной код бэктеста без изменений
        setup_logging(cfg)
        ...
        try:
            ...
            for name_result, value in metrics.items():
                ...
                try:
                    ...
                    # Если не получается преобразовать в число, логируем как тег
                    mlflow.set_tag(name_result, value_str)
            ...
            if cfg.backtest.plot_backtest_balance_curve:
                ...
                logging.info(f"[MLflow] Logged balance curve to {balance_curve_path}")

            # Сохранение обученной модели
            if os.path.exists(model_path):
                mlflow.log_artifact(model_path, "models")
                logging.info(f"[MLflow] Logged model to {model_path}")
            ...
            logging.info(f"[MLflow] Logged config to {cfg_path}")
            ...
            logging.error(f"[MLflow] Failed to log results: {str(e)}")
