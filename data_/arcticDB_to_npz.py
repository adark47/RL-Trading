...
class FinancialDataPreprocessor:
    ...
    def add_technical_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Обработка NaN значений - лучше использовать dropna() вместо заполнения нулями
        initial_count = len(df)
        df = df.dropna()
        final_count = len(df)

        if initial_count != final_count:
            self.logger.warning(f"⚠️ Удалено {initial_count - final_count} строк с NaN значениями")

        # Обновляем список feature_columns с новыми индикаторами
        new_features = ['macd', 'macd_signal', 'macd_hist', 'rsi', 'bb_upper', 'bb_mid', 'bb_lower', 'hma_fast',
                        'hma_slow', 'atr']
...
if __name__ == "__main__":
    ...
    try:
        ...
        TIMEFRAME = "1m"
        MARKET_TYPE = "linear"
        DAYS_BACK = 2.0  # Глубина запроса в днях
